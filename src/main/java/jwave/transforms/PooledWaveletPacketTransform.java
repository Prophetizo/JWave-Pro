        ...
        try {
            while (h >= transformWavelength && l < level) {
                ...
                for (int p = 0; p < g; p++) {
                    ...
                    int offset = p * h;
                    System.arraycopy(arrHilb, offset + 0, iBuf, 0, h);

                    double[] oBuf = _wavelet.forward(iBuf, h);

                    System.arraycopy(oBuf, 0, arrHilb, offset + 0, h);
                }
        ...
        try {
            while (h <= arrTime.length && h >= transformWavelength) {
                ...
                for (int p = 0; p < g; p++) {
                    ...
                    int offset = p * h;
                    System.arraycopy(arrTime, offset + 0, iBuf, 0, h);

                    double[] oBuf = _wavelet.reverse(iBuf, h);

                    System.arraycopy(oBuf, 0, arrTime, offset + 0, h);
                }
